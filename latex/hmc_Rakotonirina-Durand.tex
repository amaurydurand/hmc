...
\begin{Prop}[conservation du hamiltonien]\label{prop:conservation}
  Le hamiltonien est conservé sur la trajectoire i.e si $z$ est solution de \eqref{eq:edo} alors
  $$
  \forall t \in \rset_+, (H \circ z)'(t) = 0.
  $$
\end{Prop}
...
\end{Lem}
\begin{proof}
  On peut écrire $\bar{z} = u \circ z \circ v$ avec $u : (x,p) \mapsto (x, -p)$ et $v : t \mapsto -t$. Alors en notant $\frac{\partial u}{\partial z}$ la matrice jacobienne de $u$, 
  $$
  \forall t \in \rset_-, \bar{z'}(t) = - (u \circ z)'(-t) = - \frac{\partial u}{\partial z}(z(-t)) z'(-t) = -
  \begin{bmatrix}
    I_{d} & 0_{d} \\
    0_{d} & - I_{d}
  \end{bmatrix}
